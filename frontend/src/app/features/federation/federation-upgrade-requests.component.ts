...
export class FederationUpgradeRequestsComponent implements OnInit {
  ...
  load() {
    ...
    this.api.getFederationUpgradeRequests({ status, page: this.page, pageSize: this.pageSize })
      .subscribe({
        next: r => {
  ...
  approve(item: any) {
    this.busy.set(true);
    this.api.approveFederationUpgradeRequest(item.id).subscribe({
  ...
  confirmReject() {
    ...
    if (!item) return;
    this.busy.set(true);
    this.api.rejectFederationUpgradeRequest(item.id, this.rejectReason || undefined).subscribe({
