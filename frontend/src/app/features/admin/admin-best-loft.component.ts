...
@Component({
  selector: 'app-admin-best-loft',
  standalone: true,
  imports: [CommonModule, FormsModule],
  template: `
    <div class="pr-page-header">
      <h1 class="pr-page-header__title">Best Loft Results</h1>
      <p class="pr-page-header__subtitle">All best loft rankings across clubs and programmes</p>
    </div>
    ...
    <div class="pr-card mb-4">
      <div class="flex flex-wrap gap-4 items-end">
        <div class="pr-form-group" style="flex:1;min-width:200px">
          ...
          <input class="pr-input" [(ngModel)]="search" placeholder="Fancier or club name…" (keyup.enter)="loadPage(1)">
        </div>
      ...
      @if (loading()) {
        ...
        <div class="pr-table-wrapper">
          <table class="pr-table">
            <thead>
              <tr>
                <th>Rank</th><th>Fancier</th><th>Programme</th><th>Year</th>
                <th>Club</th><th>Score</th><th>Races</th><th>Actions</th>
              </tr>
            </thead>
            <tbody>
              @for (r of items(); track r.id) {
                <tr>
                  <td><span class="pr-badge">{{ r.rank }}</span></td>
                ...
                <tr><td colspan="8" class="text-center text-muted py-6">No best loft results found</td></tr>
              }
            </tbody>
          </table>
        </div>
...
export class AdminBestLoftComponent implements OnInit {
  ...
  loadPage(p: number) {
    ...
    this.api.adminGetBestLoftResults(params).subscribe({
      next: r => {
        const data = r?.data ?? r;
      ...
      error: () => { this.error.set('Failed to load best loft results.'); this.loading.set(false); }
    });
  }
  ...
  viewResult(r: any) {
    this.router.navigate(['/club/programmes', r.programmeId, 'best-loft']);
  }
  ...
  downloadPdf(r: any) {
    ...
    this.error.set(null);
    this.print.renderBestLoftResultsPdf({ programmeId: r.programmeId, designId: 'L1', language: this.i18n.locale() }).subscribe({
      next: blob => {
        const safe = (r.programmeName ?? 'programme').replace(/[^a-z0-9-]+/gi, '_');
        this.print.download(blob, `best-loft-${safe}-${r.programmeYear}.pdf`);
