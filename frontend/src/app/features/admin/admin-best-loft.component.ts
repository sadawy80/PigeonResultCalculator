...
@Component({
  ...
  template: `
    <div class="pr-page-header">
      <h1 class="pr-page-header__title">{{ 'admin.bestLoft.title' | translate }}</h1>
      <p class="pr-page-header__subtitle">{{ 'admin.bestLoft.subtitle' | translate }}</p>
    </div>
    ...
    <div class="pr-card mb-4">
      <div class="flex flex-wrap gap-4 items-end">
        <div class="pr-form-group" style="flex:1;min-width:200px">
          ...
          <input class="pr-input" [(ngModel)]="search" [placeholder]="'admin.bestLoft.searchPlaceholder' | translate" (keyup.enter)="loadPage(1)">
        </div>
          ...
          <table class="pr-table">
            ...
            <tbody>
              @for (r of items(); track r.id) {
                ...
                <tr><td colspan="8" class="text-center text-muted py-6">{{ 'admin.bestLoft.noResults' | translate }}</td></tr>
              }
            </tbody>
          </table>
        </div>
...
export class AdminBestLoftComponent implements OnInit {
  ...
  loadPage(p: number) {
    ...
    this.api.adminGetBestLoftResults(params).subscribe({
      ...
      error: () => { this.error.set(this.i18n.t('admin.bestLoft.loadFailed')); this.loading.set(false); }
    });
  }
