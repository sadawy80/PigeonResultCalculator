...
@Component({
  ...
  template: `
    <div class="pr-page-header">
      <h1 class="pr-page-header__title">{{ 'admin.superAce.title' | translate }}</h1>
      <p class="pr-page-header__subtitle">{{ 'admin.superAce.subtitle' | translate }}</p>
    </div>
    ...
    <div class="pr-card mb-4">
      <div class="flex flex-wrap gap-4 items-end">
        <div class="pr-form-group" style="flex:1;min-width:200px">
          ...
          <input class="pr-input" [(ngModel)]="search" [placeholder]="'admin.superAce.searchPlaceholder' | translate" (keyup.enter)="loadPage(1)">
        </div>
          ...
          <table class="pr-table">
            ...
            <tbody>
              @for (r of items(); track r.id) {
                ...
                <tr><td colspan="10" class="text-center text-muted py-6">{{ 'admin.superAce.noResults' | translate }}</td></tr>
              }
            </tbody>
          </table>
        </div>
...
export class AdminSuperAceComponent implements OnInit {
  ...
  loadPage(p: number) {
    ...
    this.api.adminGetSuperAceResults(params).subscribe({
      ...
      error: () => { this.error.set(this.i18n.t('admin.superAce.loadFailed')); this.loading.set(false); }
    });
  }
  ...
  viewResult(r: any) {
    this.router.navigate(['/club/programmes', r.programmeId, 'super-ace-pigeon']);
  }
  ...
  downloadPdf(r: any) {
    ...
    this.error.set(null);
    this.print.renderSuperAceResultsPdf({ programmeId: r.programmeId, designId: 'SA1', language: this.i18n.locale() }).subscribe({
      next: blob => {
        const safe = (r.programmeName ?? 'programme').replace(/[^a-z0-9-]+/gi, '_');
        this.print.download(blob, `super-ace-${safe}-${r.programmeYear}.pdf`);
