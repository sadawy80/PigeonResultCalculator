# Ace Pigeon Result — Production Spec

`ace_result_prod.html` — A4 portrait, multi-page table, 13 designs (10 EN + 3 AR), EN+AR languages. Renders an Ace Pigeon ranking across many races, with sub-rows per race per fancier.

## Files

- **`ace_result_prod.html`** — the template
- This spec document
...
3. `window.ACE_DATA = {...}` — **preferred for headless rendering**
4. `window.postMessage({type:'ace_data', payload:{...}})` — iframe embed

## URL params and window globals

- `?design=A1..A10` or `AR-A1..AR-A3` (default `A1`)
- `?lang=en|ar` (Arabic designs `AR-*` auto-force `lang=ar`)
- `?print=1` — auto-`window.print()` on render complete
- `window.ACE_DESIGN`, `window.ACE_LANG`, `window.ACE_AUTOPRINT` — alternatives
...
```json
{
  "meta": {
    ...
    "org_ar": "اسم النادي",
    "eyebrow_en": "Ace Pigeon Result",
    "eyebrow_ar": "نتيجة أفضل طائر",
...
| `eyebrow_<lang>` | string | "Ace Pigeon Result" line above org name |
...
**English (10):** A1, A2, A3, A4, A5, A6, A7, A8, A9, A10
**Arabic (3):** AR-A1, AR-A2, AR-A3

Same design families as race results — Classic Cream (A1), Royal Navy & Gold (A2), Editorial Black (A3), Swiss Minimal (A4), Carbon Tech (A5), Inter Bold (A6), Imperial Gold (A7), Burgundy Elite (A8), Platinum Edge (A9), Stadium Roster (A10). Arabic: Kaaba Gold (AR-A1), Ivory Calligraphic (AR-A2), Kufi Mihrab (AR-A3).
...
var injection = $$"""
<script>
  window.ACE_DESIGN = '{{designId}}';
  window.ACE_LANG = '{{lang}}';
  window.ACE_DATA = {{JsonSerializer.Serialize(data, _jsonOpts)}};
</script>
""";
